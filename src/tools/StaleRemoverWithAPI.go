...
func main() {
	...
	if oldAssetURL != "" {
		oldAssetURL = oldAssetURL + "/itswebapi/v1/GetEndpointsUninstalled/%d/%s"
	} else {
	...
	mapPartnerIDListEndpoints := processForMailBoxAndLegacy(uniquePartnerIDs)
...
func getCassandraSession(assetClusterIP, agentClusterIP string) error {
	...
	agentCluster := gocql.NewCluster(clusterIP)
