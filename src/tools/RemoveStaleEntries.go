...
const (
	...
	queryToInsertAssetInActive       = `INSERT INTO platform_asset_db.asset_partner_endpoint_details_inactive(partner_id,client_id,site_id,endpoint_id,reg_id,dc_ts_utc,agent_ts_utc,created_by,name,type,remote_address,resource_type,baseboard,bios,drives,physical_memory,networks,os,processors,raidcontroller,system, installed_softwares, keyboards, mouse, monitors, physical_drives,users,services,shares) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
...
func main() {
	...
	if len(commandArgs) != 2 {
		fmt.Println("Usage: YourScriptName <config.json> <LPR.xlsx>")
		fmt.Println("Example: RemoveStaleEntries config.json LPR.xlsx")
		return
	}
	startTime := time.Now()
	...
	isFile := configObject.PickUninstallListFromFile
	partnerIds := configObject.PartnerIds
	thresholdDays := configObject.ThresholdDays
	var uniquePartnerIDs []string
	...
	//http://rmmitswebapi.itsupport247.local/v1/GetEndpointsUninstalled/NumofDays/MemberID

	if oldAssetURL != "" {
		oldAssetURL = oldAssetURL + "/v1/GetEndpointsUninstalled/%d/%s"
	} else {
		fmt.Println("Old Asset URL is Empty. Stoping Script.")
	}

	if isFile {
		mapPartnerIDRegIDRemoved, _ = readEndpoints(os.Args[2])
		partnerIDs, err := ReadPartnersFromExcel(excelPath, "sheetName")
		//partnerIDs, err := GetPartnerIDs()
		if err != nil {
			fmt.Println("Error Occured while getting the partners from Cassandra, Error : ", err)
			return
		}
		lengthOfTotalPartnerID = len(partnerIDs)
		...
		legacystartTime := time.Now()
		mapPartnerIDRegIDRemoved, uniquePartnerIDs, mapPartnerIDSiteIDs, err = readEndpointsByAPI(partnerIds, thresholdDays)
		legacyEndTime := time.Since(legacystartTime)
		fmt.Printf("Total Time Taken to get the Uninstalled Agents from 1.o is : (%v)", legacyEndTime)
	} //junoValue, err := GetJunoData(values)
	//logger.Println("Agents Removed are : ", mapPartnerIDRegIDRemoved)
	...
	//Get the Map for different Cases of heartbeat and installed flag
	//mapPartIDHbTrueFalseAndInstTrue, mapPartIDHbTrueAndInstFalse, mapPartIDHbFalseAndInstFalse = getTheHeartbeatAndInstalledMap(uniquePartnerIDs)
	//will get the list of endpoints to be removed by partner Id
	startTmForMailBox := time.Now()
	mapPartnerIDListEndpoints := processEndpointsFromLegacy(uniquePartnerIDs)
	endTimeForMailbox := time.Since(startTmForMailBox)

	fmt.Println("Total time Taken to Identify Entries for Deletion are : ", endTimeForMailbox)
	//TODO process the case of FF Condition for deletion
	...
	for partID, endpoints := range mapPartnerIDListEndpoints {
		totalEndpoints = totalEndpoints + len(endpoints)
		fmt.Printf("\n  %s | %d | %v \n", partID, len(endpoints), endpoints)
	}
	fmt.Println("\n****** Identified Entries END  **********")

	//processDeletionAndPostMailBoxMessages(mapPartnerIDListEndpoints, mapPartIDHbTrueFalseAndInstTrue, mapPartIDHbTrueAndInstFalse, mapPartIDHbFalseAndInstFalse, doDelete)

	// endTime := time.Since(startTime)
	// fmt.Println("Identification completed in : ", endTime)
	// fmt.Printf("\n\n*** After Adding false cases Metrix appears as ***\n")
	fmt.Println("Is Delete option selected : ", doDelete)

	// fmt.Println("Total Stale Entries Identified (Adding the false/null flags) are")
	// fmt.Println("Partner ID | Total no. entries | Endpoint IDs")
	// for partID, endpoints := range mapPartnerIDListEndpoints {
	// 	totalEndpoints = totalEndpoints + len(endpoints)
	// 	fmt.Printf("\n  %s | %d | %v \n", partID, len(endpoints), endpoints)
	...
	fmt.Printf("\n \n Total No. Stale Entries Identified are : %d", totalEndpoints)
	AssetSession.Close()
	AgentSession.Close()
}
	...
	for partnerID, endpointIDs := range mapPartnerIDListEndpoints {
		...
		for a = 0; a < lengthOfStale; a++ {
			...
			if assetCollection.EndpointID != "" {
				...
				if err != nil {
					fmt.Printf("\n Error Occured while inserting the Asset in inactive, following is the asset %v amnd err : %v \n", assetCollection, err)
				}
			}
		}
		close(results)
		...
		if err != nil {
			fmt.Printf("\nError Occured while deleting the stale entries from Asset Collection for Partner Id : %v, err : %v \n ", partnerID, err)
		}
		err = deleteEndpointsFromVersion(endpointIDs, partnerID)
		if err != nil {
			fmt.Printf("\n Error Occured while deleting the stale entries from Agent Version Details for Partner Id : %v and err : %v \n", partnerID, err)
		}
...
func processEndpointsFromLegacy(uniquePartnerIDs []string) map[string][]string {
