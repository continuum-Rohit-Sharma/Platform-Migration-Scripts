...
const (
	insertEndpointManifest = "INSERT INTO platform_agent_db.endpoint_manifest (partner_id,manifest_version,endpoint_id, dc_created_ts_utc,requested_on_ts_utc) VALUES(?,?,?,?,?);"
)
...
func main() {
	...
	for cnt, endpoint := range endpoints {
		if (cnt > offset) && (cnt <= offset+noOfWorkers) {
			...
			partnerIDEndpointIDs[endpoint.partnerID] = append(partnerIDEndpointIDs[endpoint.partnerID], endpoint.endpointID)
			newOffset = cnt
		}
	}
	fmt.Println(partnerIDEndpointIDs)
	for partnerID, _ := range partnerIDs {
		err = saveManifestVersionForPartner(manifestVersion, partnerID, partnerIDEndpointIDs[partnerID])
		if err != nil {
...
func saveManifestVersionForPartner(manifestVersion string, partnerID string, endIds []string) error {
	logfile, err := os.OpenFile("ErrorLogs.log", os.O_CREATE|os.O_RDWR|os.O_APPEND, 0644)
	if err != nil {
		fmt.Printf("\n Error occurred while creating the log file\n Err  : %v", err)
		return err
	}
	logr := log.New(logfile, "", logFlags)
	if manifestVersion == "" || partnerID == "" {
		return errors.New("Blank manifest or partner ID")
	}
	partnerID = strings.TrimSpace(partnerID)
	for _, endpointID := range endIds {
		err = Session.Query(insertEndpointManifest, partnerID, manifestVersion, endpointID, time.Now().UTC(), time.Now().UTC()).Exec()
		if err != nil {
			logr.Printf("\n Error Occurred : %v, while executing the query : %v, partnerID : %s & endpointid : %s \n", err, insertEndpointManifest, partnerID, endpointID)
		}
	}
